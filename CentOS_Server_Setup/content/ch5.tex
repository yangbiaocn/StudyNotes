\chapter{Shadowsocks + obfsproxy 混淆翻墙的实现}
Shadowsocks 的作者 clowwindy 被喝茶了，并被迫删除了 Github 上的源码。
这意味着 ss 以后不会得到更新，其流量特征也许会被不断进化的 GFW 识别并封锁。
故使用 obfsproxy 进行二次混淆，降低被识别的风险。

以下步骤均在 Debian 7 32 位系统上进行。

\section{配置 shadowsocks-libev 服务端}
参考官方文档[ https://github.com/shadowsocks/shadowsocks-libev ]，这里从源直接安装。
添加 GPG public key
...
\end{lstlisting}
\end{colorboxed}
server 填写 0.0.0.0 允许所有设备连接，填写 127.0.0.1 则仅允许本机内环连接。加密方式一般选择 aes-256-cfb。
...
\end{colorboxed}
启动ss服务端。
\section{配置 shadowsocks 客户端（Windows 平台）}
